...
export function DatasetChips(props: {
  ctx: ViewContext, legislation: string[],
}) {
  const { ctx, legislation } = props;
  return (
    <>
      {reader.containsOpenData(legislation) ?
        <li> <OpenDataChip ctx={ctx} /></li> : null}
      {reader.containsPublicRegistry(legislation) ?
        <li> <PublicRegistryChip ctx={ctx} /></li> : null}
      {reader.containsHighValueDataset(legislation) ?
        <li> <HighValueDatasetChip ctx={ctx} /></li> : null}
      {reader.containsDynamicData(legislation) ?
        <li> <DynamicDataChip ctx={ctx} /> </li> : null}
      {reader.containsNonPublicData(legislation) ?
        <li> <NonPublicChip ctx={ctx} /></li> : null}
    </>
  )
}

const reader = createApplicableLegislation();
